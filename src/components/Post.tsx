  ...
  if (isPopup) {
    return (
      <>
        ...
        >
          <div
            className="flex min-h-[8rem] items-center justify-center px-6 py-7"
            style={{
              backgroundColor: resolvedBrandColour,
              color: brandTextColour,
            }}
          ...
          <div className="p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    {isEloPost ? "ELO Pop-Up · Today" : "Pop-Up · Today"}
                  </span>
                  {isLocalPartner && (
              ...
              <ArrowRight
                className="mt-1 h-5 w-5 shrink-0 transition duration-300 group-hover:translate-x-1"
                style={{ color: resolvedBrandColour }}
              />
            </div>
            ...
            <div className="mt-5 space-y-3 text-sm text-slate-500">
              {metadata?.popup_address ? (
                <div className="flex items-start gap-2">
                  <MapPin className="mt-0.5 h-4 w-4 shrink-0" />
                  <span>{metadata.popup_address}</span>
                </div>
              ) : null}
              ...
              {(metadata?.popup_start_time || metadata?.popup_end_time) ? (
                <div className="flex items-center gap-2">
                  ...
                  <span>
                    Today · {metadata?.popup_start_time || "?"} –{" "}
                    {metadata?.popup_end_time || "?"}
                  </span>
                </div>
              ) : null}
              ...
              <div className="mt-5 border-t border-dashed border-slate-200 pt-5">
                ...
                {updateImages.length > 0 ? (
                  <div className={`mt-6 grid gap-2 ${
                    updateImages.length === 1 ? "grid-cols-1" :
                    updateImages.length === 2 ? "grid-cols-2" :
